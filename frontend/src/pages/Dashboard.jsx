...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Listen for expense changes from anywhere in the app
    const handleUpdated = () => {
      setIsLoading(true);
      fetchData();
    };
    ...
    <div className="space-y-10 relative" ref={containerRef}>
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8 pb-4 border-b border-white/5">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-sm text-text-tertiary mt-1 font-medium tracking-wide italic">
              A clear view of your money for this month.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col items-start md:items-end gap-2 px-6 py-4 rounded-[2rem] bg-brand-500/5 border border-brand-500/10 backdrop-blur-md shadow-2xl shadow-brand-500/5">
          <p className="text-[10px] text-brand-500 font-black uppercase tracking-[0.4em] mb-1">Our focus</p>
          ...
          >
            <Card className="dash-card glass-card">
              <CardContent className="py-24 text-center">
                <h3 className="text-2xl font-bold text-text-secondary tracking-tight">No spending data yet</h3>
                <p className="mt-3 text-sm text-text-tertiary max-w-md mx-auto">Add your income and expenses to see forecasts, your health score, and personalized tips.</p>
              </CardContent>
            </Card>
          </motion.div>
        ) : (
                  ...
                  <div className="flex flex-col md:flex-row items-start gap-8">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-3">
                         <p className="text-[10px] text-brand-500 font-black uppercase tracking-[0.3em]">AI monthly summary</p>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4 relative z-10">
                  ...
                  <CardContent className="p-6 relative overflow-hidden">
                    ...
                    <div className="flex items-center justify-between mb-4 relative z-10">
                      ...
                      <div className="flex items-center gap-1 opacity-40 group-hover:opacity-100 transition-opacity">
                        ...
                        <span className="text-[8px] font-black text-brand-500 uppercase tracking-tighter">Updated</span>
                      </div>
                    </div>
              ...
              <Card className={`dash-card glass-card border-none group bg-gradient-to-br ${scoreColor.bg} hover:bg-white/[0.05] transition-all duration-500`}>
                <CardContent className="p-6 relative overflow-hidden">
                  ...
                  <dl className="relative z-10">
                    <dt className="text-[10px] font-black text-text-tertiary uppercase tracking-widest pl-1">Financial health score</dt>
                      ...
                      <div className="space-y-6">
                        <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/5">
                          <p className="text-text-secondary text-sm leading-relaxed font-medium capitalize">
                            Status: <strong className="text-text-primary font-black ml-1">{healthScore.status}</strong>
                          </p>
                        </div>
              ...
              <Card className="dash-card glass-card border-none lg:col-span-2">
                <CardHeader className="pb-4">
                  <CardTitle className="text-lg font-black tracking-tight text-text-primary flex justify-between items-center px-1">
                    Financial History
                    <span className="text-[9px] font-black text-text-tertiary uppercase tracking-widest bg-white/5 px-2 py-1 rounded-md">Tap to expand</span>
                  </CardTitle>
                </CardHeader>
                            ...
                            <div className="flex items-center gap-6 relative z-10">
                              <div className="text-right hidden sm:block">
                                 ...
                                 <div className="flex items-center justify-end gap-1">
                                   ...
                                   <p className="text-[9px] text-text-tertiary font-bold uppercase tracking-widest">Spent</p>
                                 </div>
                              </div>
                              <motion.div 
                              ...
                              >
                                <ul className="px-6 pb-6 pt-4 space-y-3">
                                  {record.entries.map((entry, idx) => {
                                    const expenseVals = Object.values(entry.expenses || {});
                                    const debit = expenseVals.reduce((a, b) => a + Number(b || 0), 0);
                                    const credit = Number(entry.income ?? 0);
                                    return (
                                      <motion.li 
                                        key={idx}
                                        ...
                                        className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pl-12 pr-6 py-4 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-brand-500/20 transition-all group/entry hover:bg-white/[0.05]"
                                      >
                                        ...
                                        <div className="flex items-end sm:items-center justify-end gap-6 sm:gap-8 shrink-0">
                                          <div className="text-right">
                                            <span className="text-sm font-black text-emerald-400/90 tracking-tighter block tabular-nums">₹{credit.toLocaleString('en-IN', { maximumFractionDigits: 2 })}</span>
                                            <div className="text-[8px] text-text-tertiary font-black uppercase tracking-widest">Credit</div>
                                          </div>
                                          <div className="h-8 w-px bg-white/10 hidden sm:block" aria-hidden />
                                          <div className="text-right">
                                            <span className="text-sm font-black text-text-primary tracking-tighter block tabular-nums group-hover/entry:text-brand-400 transition-colors">₹{debit.toLocaleString('en-IN', { maximumFractionDigits: 2 })}</span>
                                            <div className="text-[8px] text-text-tertiary font-black uppercase tracking-widest">Debit</div>
                                          </div>
                                        </div>
                                      </motion.li>
                                    );
