    ...
    <AnimatePresence mode="wait">
      {isLoading ? (
        <motion.div 
          ...
          className="bg-black/20 border border-white/5 rounded-[2rem] p-6 animate-pulse"
        >
        ...
        <motion.div
          ...
          className="bg-black/20 border border-white/5 rounded-[2rem] p-7 relative overflow-hidden group"
        >
          {/* Ambient glow */}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 relative z-10">
            ...
            <div className="sm:col-span-1 flex flex-col items-center justify-center bg-white/[0.02] rounded-2xl p-5 border border-white/5">
              <svg className="w-16 h-16 -rotate-90" viewBox="0 0 64 64">
                ...
                <circle
                  cx="32" cy="32" r="28"
                  fill="none"
                  stroke={Math.max(0, Math.min(100, data.health_score)) >= 70 ? '#d4af37' : Math.max(0, Math.min(100, data.health_score)) >= 40 ? '#f59e0b' : '#ef4444'}
                  strokeWidth="5"
                  strokeLinecap="round"
                  strokeDasharray={`${(Math.max(0, Math.min(100, data.health_score)) / 100) * (2 * Math.PI * 28)} ${2 * Math.PI * 28}`}
                  className="transition-all duration-1000 ease-out"
                />
              </svg>
              <div className="-mt-12 text-center">
                <p className={`text-xl font-black tracking-tight italic ${STATUS_COLOR[data.health_status] || 'text-text-tertiary'}`}>{data.health_score}</p>
                <p className="text-[8px] text-text-tertiary font-black uppercase tracking-widest">/100</p>
              </div>
              <p className={`text-[9px] font-black uppercase tracking-widest mt-2 ${STATUS_COLOR[data.health_status] || 'text-text-tertiary'}`}>{data.health_status}</p>
            </div>
            ...
            <div className="bg-white/[0.02] rounded-2xl p-5 border border-white/5 flex flex-col justify-between">
              ...
              <div className="w-full bg-black/30 rounded-full h-1 mt-3 overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{ width: `${Math.min(100, (data.total_expense / (data.total_income || 1)) * 100)}%` }}
            ...
            {/* Savings + Trend */}
            <div className={`${(TREND_CONFIG[data.trend] || TREND_CONFIG.stable).bg} rounded-2xl p-5 border ${(TREND_CONFIG[data.trend] || TREND_CONFIG.stable).border} flex flex-col justify-between`}>
              <div className="flex items-center justify-between mb-2">
                <p className="text-[9px] text-text-tertiary font-black uppercase tracking-widest">Savings</p>
                <div className={`flex items-center gap-1 text-[9px] font-black uppercase tracking-widest ${(TREND_CONFIG[data.trend] || TREND_CONFIG.stable).color}`}>
                  {(function() { const Icon = (TREND_CONFIG[data.trend] || TREND_CONFIG.stable).icon; return <Icon className="w-3 h-3" />; })()}
                  {(TREND_CONFIG[data.trend] || TREND_CONFIG.stable).label}
                </div>
              </div>
              ...
              <p className={`text-[10px] font-black italic mt-2 ${(TREND_CONFIG[data.trend] || TREND_CONFIG.stable).color}`}>
